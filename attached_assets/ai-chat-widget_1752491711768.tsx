...
const AIChatWidget: React.FC<AIChatWidgetProps> = ({
  ...
  onToggle,
  phoneNumber,
  apiEndpoint = 'https://mzhyi8cqp3dk.manus.space/api/ai'
}) => {
  ...
  const initializeChat = async () => {
    try {
      ...
      const greeting = await getLocalizedGreeting(locationData.location?.country || 'US');
  ...
  const getLocalizedGreeting = async (countryCode: string): Promise<string> => {
    try {
      const response = await fetch(`${apiEndpoint}/translate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: "Hello! Welcome to RocVille Media House. I'm your AI assistant ready to help with web development, digital marketing, branding, and more. What can I help you with today?",
          target_lang: getLanguageFromCountry(countryCode),
          source_lang: 'en'
        })
      });
      ...
      return "Hello! Welcome to RocVille Media House. I'm your AI assistant. How can I help you today?";
    }
  };
  ...
  const getLanguageFromCountry = (countryCode: string): string => {
    const countryToLanguage: { [key: string]: string } = {
      ...
      'RU': 'ru', 'JP': 'ja', 'KR': 'ko', 'CN': 'zh', 'AR': 'ar',
      'IN': 'hi', 'TH': 'th', 'VN': 'vi', 'ID': 'id', 'MY': 'ms',
      'PH': 'tl', 'BR': 'pt', 'MX': 'es', 'NG': 'en', 'KE': 'sw'
    };
